...
最后，将所有修改（包括配置文件）上传到 GitHub（排除密钥文件），并进行严格的运行验证。
...
### 5. 上传机制与严格验证规则 (新增强制要求)
- **What**: 将所有代码、配置文件上传到 GitHub，排除密钥文件，并进行严格的运行验证。
- **How**:
  - 检查 `.gitignore`，确保仅包含系统级忽略和密钥文件（如 `.github_token` 等），其他诸如 `.cursorrules` 或相关配置文件应当保留在版本控制中（如果适用）。
  - 使用 `git commit` 将代码提交。
  - **强制验证步骤**：修改完成后，必须对引入的新函数、UI进行逻辑分析或单元测试，确保不会出现空值返回、未定义函数崩溃、或渲染异常的情况。
  - 如果在静态分析或测试中仍然发现潜在崩溃或缺失函数的问题，必须将其详细输出到报错日志中（如创建 `error_log.txt` 或在回复中明确指出），不能隐瞒。
...
3. **验证代码完整性**：
   - 使用 `flutter analyze` 等命令确保无未定义的函数或语法错误。
   - 确保没有可能导致空指针的未处理可空变量。