...
- **Dart 终极诊断脚本**：提供一个仅依赖 `dart:io` 的网络与 DNS 诊断函数，用于在应用启动时快速区分是系统 DNS 瘫痪还是 App 缺乏底层网络权限，同时通过 `NetworkInterface.list()` 输出当前活跃的网络通道（如 WIFI、VPN、蜂窝数据），排查流量走向和网卡绑定问题。如果在诊断中发现网络异常，所有详细的诊断数据都将完整记录并输出到本地报错日志文件中（例如 `webdav_error_log.txt` 或标准开发日志）。
