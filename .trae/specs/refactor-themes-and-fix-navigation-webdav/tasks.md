...
- [x] Task 7: 全局自检。
  - [x] SubTask 7.1: 在前面所有任务完成后，全局检查所有代码、业务逻辑与 UI 溢出情况，修复潜在问题。

- [x] Task 8: 自动化提交、编译、纠错与发布 v1.2.2。
  - [x] SubTask 8.1: 使用 Git 提交代码至仓库，触发 GitHub Actions 编译。
  - [x] SubTask 8.2: 监控编译报错并自动修改代码，循环重试直至成功。
  - [x] SubTask 8.3: 成功后发布为 `1.2.2` 正式版本（Release 描述仅包含当前版本的更新日志）。