...
- **同步引擎优化**：云端文件对比逻辑放弃了不稳定的最后修改时间，改为仅校验文件大小，有效修复了因上传导致时间戳更新从而重复下载大文件的问题。
- **首页刷新修正**：移除了在本地文件加密后过早将其录入到云端同步索引的错误逻辑。现在，当你添加新的加密文件时，点击首页刷新按钮将准确显示增加的“差异文件”，直到完成同步后“云端文件”数量才会增加。
