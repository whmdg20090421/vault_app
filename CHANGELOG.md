...
* **FIX**: 彻底修复全局透明背景状态下页面滑动返回时的卡顿和重叠问题（通过移除路由动画快照和应用 Zoom 动画）
* **FEAT**: 在加密/解密任务进度页面加入针对单个错误文件的“重试”按钮和针对全部错误文件的“一键重试”按钮
* **FIX**: 持续提供并验证此前对双斜杠路径 (`//`) 导致 Android 系统底层抛出 PathNotFoundException (OS Error 2) 的拦截处理

# 版本 1.5.0 (2026-04-21)

* **FEAT**: 重构长按加密文件夹的弹出菜单操作，以居中实体框方式显示，解决了背景透明重叠导致的文字冲突问题
* **FEAT**: 针对应用的深色、浅色及赛博朋克主题优化，添加适配各自主题的全局路由切换与平滑反馈动画
* **FIX**: 修复云盘同步目录选择时无数据和不显示的问题，并增加手动下拉刷新云端文件数（无限深度）功能
* **FIX**: 修复部分操作系统（如 Android）下由路径冗余斜杠 `//` 导致的打开 JSON 或解密预览文件时报错 `PathNotFoundException` (OS Error 2) 的问题
* **CHORE**: 升级代码构建规范，将包含相关构建监测与配置文件同步的指导规则写入并整合至 `.trae/rules.md`

# 版本 1.4.9 (2026-04-21)

* **FIX**: 修复旧版保险箱升级时点击升级选项闪退或无响应的问题
* **FEAT**: 支持将旧版保险箱更新并重新加密，提供全屏加密进度及灵动岛悬浮胶囊，并增加防退出的二次警告
