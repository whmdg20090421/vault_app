...
- **V4 Encryption Architecture**: 彻底重构加密任务处理模块，基于“最终完整版V4”技术需求，实现全新的数据模型、多线程引擎、状态机以及动态进度刷新与 UI 呈现。
- **Tree-based Task Management**: 实现了基于树状结构 (`EncryptionNode`) 的全局任务列表，支持导入文件夹后无限层级的嵌套展开与状态独立管理，并持久化至应用私有数据目录。
- **Multi-threaded Executor**: 基于 Isolate 实现多线程并行加密调度池，并支持应用重启时自动重置未完成任务状态，实现中断恢复（不支持断点续传），同时隔离文件损坏等异常。
- **Dynamic UI Refresh**: 引入前后台智能刷新机制（前台 0.5~1秒高频刷新 UI，后台 5~10秒低频仅持久化数据），大幅降低高频重绘带来的性能损耗。
- **Progress Visualizations**: 在主页顶部导航栏新增小型四色进度条图标（遵循理论占比 <1% 强制保持 1px 显示规则）；并新增了半屏从下往上滑入的同步进度模态框，提供极细四色进度条、百分比统计及无限级子目录折叠展开功能。
- **Task Control & History**: 实现了条目级的级联暂停/继续控制功能与长按移除功能；任务完成后自动执行存在性检查并归档，提供专门的历史记录 UI 进行展示与管理，并在加密完成后将文件哈希关联写入本地索引库。
